...
Kamiar_Climate <- read.csv("rawdata/KAM_RCP85_spain.csv", header=TRUE)
Kamiar_NoClimate <- read.csv("rawdata/KAM_RCP26_spain.csv", header=TRUE)
Kamiar_NoClimate_Volatility <- read.csv("rawdata/KAM_RCP26_vol.csv", header=TRUE)
Kamiar_Climate_Volatility <- read.csv("rawdata/KAM_RCP85_vol.csv", header=TRUE)
...
Kamiar_Climate_Volatility_wide <- clim_data_management(Kamiar_Climate_Volatility)
Kamiar_NoClimate_Volatility_wide <- clim_data_management(Kamiar_NoClimate_Volatility)
...
Kahn85_vol_9 <- addLossSeries(df1, Kamiar_Climate_Volatility_wide, 9, 9)
Kahn85_vol_10 <- addLossSeries(df1, Kamiar_Climate_Volatility_wide, 10, 9)
Kahn85_vol_29 <- addLossSeries(df1, Kamiar_Climate_Volatility_wide, 29, 9)
Kahn85_vol_30 <- addLossSeries(df1, Kamiar_Climate_Volatility_wide, 30, 9)
Kahn85_vol_49 <- addLossSeries(df1, Kamiar_Climate_Volatility_wide, 49, 9)
Kahn85_vol_50 <- addLossSeries(df1, Kamiar_Climate_Volatility_wide, 50, 9)
Kahn85_vol_79 <- addLossSeries(df1, Kamiar_Climate_Volatility_wide, 79, 9)
Kahn85_vol_80 <- addLossSeries(df1, Kamiar_Climate_Volatility_wide, 80, 9)

Kahn26_vol_9 <- addLossSeries(df1, Kamiar_NoClimate_Volatility_wide, 9, 1)
Kahn26_vol_10 <- addLossSeries(df1, Kamiar_NoClimate_Volatility_wide, 10, 1)
Kahn26_vol_29 <- addLossSeries(df1, Kamiar_NoClimate_Volatility_wide, 29, 1)
Kahn26_vol_30 <- addLossSeries(df1, Kamiar_NoClimate_Volatility_wide, 30, 1)
Kahn26_vol_49 <- addLossSeries(df1, Kamiar_NoClimate_Volatility_wide, 49, 1)
Kahn26_vol_50 <- addLossSeries(df1, Kamiar_NoClimate_Volatility_wide, 50, 1)
Kahn26_vol_79 <- addLossSeries(df1, Kamiar_NoClimate_Volatility_wide, 79, 1)
Kahn26_vol_80 <- addLossSeries(df1, Kamiar_NoClimate_Volatility_wide, 80, 1)
...
df1 <- inner_join(df1, Kahn85_vol_9, by=c("CountryName", "ISO2", "Year"))
df1 <- inner_join(df1, Kahn85_vol_10, by=c("CountryName", "ISO2", "Year"))
df1 <- inner_join(df1, Kahn85_vol_29, by=c("CountryName", "ISO2", "Year"))
df1 <- inner_join(df1, Kahn85_vol_30, by=c("CountryName", "ISO2", "Year"))
df1 <- inner_join(df1, Kahn85_vol_49, by=c("CountryName", "ISO2", "Year"))
df1 <- inner_join(df1, Kahn85_vol_50, by=c("CountryName", "ISO2", "Year"))
df1 <- inner_join(df1, Kahn85_vol_79, by=c("CountryName", "ISO2", "Year"))
df1 <- inner_join(df1, Kahn85_vol_80, by=c("CountryName", "ISO2", "Year"))

df1 <- inner_join(df1, Kahn26_vol_9, by=c("CountryName", "ISO2", "Year"))
df1 <- inner_join(df1, Kahn26_vol_10, by=c("CountryName", "ISO2", "Year"))
df1 <- inner_join(df1, Kahn26_vol_29, by=c("CountryName", "ISO2", "Year"))
df1 <- inner_join(df1, Kahn26_vol_30, by=c("CountryName", "ISO2", "Year"))
df1 <- inner_join(df1, Kahn26_vol_49, by=c("CountryName", "ISO2", "Year"))
df1 <- inner_join(df1, Kahn26_vol_50, by=c("CountryName", "ISO2", "Year"))
df1 <- inner_join(df1, Kahn26_vol_79, by=c("CountryName", "ISO2", "Year"))
df1 <- inner_join(df1, Kahn26_vol_80, by=c("CountryName", "ISO2", "Year"))


fixExtremeValues <- function(frame){
	# The volatility frames sometimes for only a few countries, produce
	# extreme values as per Kahn et al 2021. This function replaces any 
	# extreme climate losses (i.e. > 100% in RCP 8.5) with the 
	# losses that are incurred in the non-volatility scenario. 

	# This only impacts the result for a small handful of countries, and
	# changing them manually might have been equally convenient. But a 
	# generalisable function works better with new data.

	iterableValues <- c(9,10,29,30,49,50,79,80)
	for (i in iterableValues){
		var_name_85 <- paste("Kahn_", as.character(i), "_9", sep="")
		var_name_conditional_replacement_85 <- paste("Kahn_", as.character(i), "_8.5", sep="")
		var_name_26 <- paste("Kahn_", as.character(i), "_1", sep="")
		var_name_conditional_replacement_26 <- paste("Kahn_", as.character(i), "_2.6", sep="")
		frame <- as.data.frame(frame %>%
		group_by(CountryName) %>%
		mutate(signal = as.numeric(!!as.name(var_name_85) < -1)) %>%
		mutate(!!as.name(var_name_85) := ifelse(signal>0, 
			!!as.name(var_name_conditional_replacement_85),
			!!as.name(var_name_85)),
			!!as.name(var_name_26) := ifelse(signal>0, 
			!!as.name(var_name_conditional_replacement_26),
			!!as.name(var_name_26))) %>%
		dplyr::select(!(signal)))}
		return (frame)
	}

df1 <- fixExtremeValues(df1)
...
df1 <- GPI_scores(df1, 10, 9)
df1 <- GPI_scores(df1, 10, 1)
df1 <- GPI_scores(df1, 30, 9)
df1 <- GPI_scores(df1, 30, 1)
df1 <- GPI_scores(df1, 50, 9)
df1 <- GPI_scores(df1, 50, 1)
df1 <- GPI_scores(df1, 80, 9)
df1 <- GPI_scores(df1, 80, 1)
...
Kahn_9_2030 <- filterFrames(df1, 2030, 9)
Kahn_9_2050 <- filterFrames(df1, 2050, 9)
Kahn_9_2070 <- filterFrames(df1, 2070, 9)
Kahn_9_2100 <- filterFrames(df1, 2100, 9)

Kahn_1_2030 <- filterFrames(df1, 2030, 1)
Kahn_1_2050 <- filterFrames(df1, 2050, 1)
Kahn_1_2070 <- filterFrames(df1, 2070, 1)
Kahn_1_2100 <- filterFrames(df1, 2100, 1)

write.csv(Baseline, file = "cleandata/Baseline_data_clean.csv")
write.csv(Baseline_longterm, file = "cleandata/Baseline_longterm_data_clean.csv")
write.csv(Kahn_8.5_2030, file = "cleandata/Kahn_RCP8.5_10_data_clean.csv")
write.csv(Kahn_2.6_2030, file = "cleandata/Kahn_RCP2.6_10_data_clean.csv")
write.csv(Kahn_8.5_2050, file = "cleandata/Kahn_RCP8.5_30_data_clean.csv")
write.csv(Kahn_2.6_2050, file = "cleandata/Kahn_RCP2.6_30_data_clean.csv")
write.csv(Kahn_8.5_2070, file = "cleandata/Kahn_RCP8.5_50_data_clean.csv")
write.csv(Kahn_2.6_2070, file = "cleandata/Kahn_RCP2.6_50_data_clean.csv")
write.csv(Kahn_8.5_2100, file = "cleandata/Kahn_RCP8.5_80_data_clean.csv")
write.csv(Kahn_2.6_2100, file = "cleandata/Kahn_RCP2.6_80_data_clean.csv")
write.csv(Kahn_9_2030, file = "cleandata/Kahn_RCP9_10_data_clean.csv")
write.csv(Kahn_9_2050, file = "cleandata/Kahn_RCP9_30_data_clean.csv")
write.csv(Kahn_9_2070, file = "cleandata/Kahn_RCP9_50_data_clean.csv")
write.csv(Kahn_9_2100, file = "cleandata/Kahn_RCP9_80_data_clean.csv")
write.csv(Kahn_1_2030, file = "cleandata/Kahn_RCP1_10_data_clean.csv")
write.csv(Kahn_1_2050, file = "cleandata/Kahn_RCP1_30_data_clean.csv")
write.csv(Kahn_1_2070, file = "cleandata/Kahn_RCP1_50_data_clean.csv")
write.csv(Kahn_1_2100, file = "cleandata/Kahn_RCP1_80_data_clean.csv")
